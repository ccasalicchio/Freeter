...
**C1. Webhook ingest endpoint on the existing MCP HTTP listener** — ✅ shipped (2026-08-13)
- `POST /ingest/{ingestToken}` on the same `127.0.0.1:39587` server (shares
  the MCP listener — the endpoint is live only while the MCP server is
  enabled). Per-widget random token generated in the widget settings;
  `allowExternal` setting already governs external reachability. No bearer
  auth on this route — the path token is the auth.
- Payload: accepts Grafana webhook contact-point format and Alertmanager
  webhook format (both are JSON with an `alerts[]` array; string status and
  `{state}` object status both handled); any other JSON is stored as one
  generic entry (body = payload JSON, capped at 500 chars).
- **C2. `alert-inbox` widget** — ✅ shipped (2026-08-13): subscribes to an
  ingest token, shows received alerts (ring buffer of 100 newest-first,
  persisted via dataStorage key `alerts`), unread badge, mark-read/clear.
- **Desktop notifications** — ✅ shipped (2026-08-13): Electron
  `Notification` in main via `src/main/infra/notifications/notifications.ts`
  + new IPC channel `freeter:show-notification`
  (`src/main/controllers/notification.ts`). Firing alerts (status
  active/firing, max 3 per request; per-widget `notifyDesktop` setting) →
  OS toast even when Freeter is in tray. The `notification` *widget API
  module* (renderer-side) is not wired yet — main enforces notifications
  directly during ingest.
...
- Security: ingest tokens are per-widget, revocable (regenerate), constant-time
  compared (`crypto.timingSafeEqual` on padded buffers); payloads size-capped
  (64 KB, request destroyed beyond) and rate-limited (10/min/token, in-memory
  sliding window, 429); invalid token/method/route all answer a uniform 404;
  never rendered as HTML (text only). Pure helpers live in
  `src/main/infra/mcpServer/ingest.ts`; ingest writes are NOT pushed onto the
  MCP undo stack.
...
| 3 | Ingest endpoint + `alert-inbox` + desktop notifications IPC — ✅ shipped (2026-08-13) | ~1 session |
